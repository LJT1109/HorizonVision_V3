<template>
    <div v-component-name="'TextNode'" :class="$style.TextNode" ref="draggable" @mousedown="dragStart">
        <div :class="$style.TextNodeBody">
            ...
            <!-- <p :class="$style.Title">TEXT{{ props.nodeProps.index }}</p> -->
            <textarea  :class="$style.InputField" v-model="textareaContent" @input="updateContent"></textarea>
...
<script setup>
import { ref, onMounted,onUpdated , defineProps, nextTick, watch } from 'vue';
...
// const textareaContent = ref(getNodeDataByIndex(props.nodeProps.index).content || "");
...
// onMounted(() => {
//     // 初始化 textareaContent 和 draggable 的位置
//     textareaContent.value = getNodeDataByIndex(props.nodeProps.index).content || "";
//     // ...其他 mounted 邏輯...
// });
...
const updateContent = () => {
    setTextNodeDataByIndex(props.nodeProps.index, { content: textareaContent.value });
};

watch(()=>getNodeDataByIndex(props.nodeProps.index).head,(newVal)=>{
    isHead.value = newVal;
    // console.log("texthead",newVal);
});
...
onMounted( () => {
    ...
    // console.log("texthead",    isHead.value);
});
